...
router.get('/:id', async (req, res) => {
    ...
    if (!id.match(/^[0-9]+$/)) {
        ...
        res.status(400).json({ error: 'id must be a positive whole number with only characters 0-9!' });
        return;
    }
    try {
        const showsData = (await axios.get(`http://api.tvmaze.com/shows/${id}`)).data;
